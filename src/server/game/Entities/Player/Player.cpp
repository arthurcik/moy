    ...
    if (m_nextSave > 0)
    {
        if (p_time >= m_nextSave)
        {
            ...
            TC_LOG_DEBUG("entities.player", "Player::Update: Player '{}' ({}) saved", GetName(), GetGUID().ToString());
                ...
                // CharacterDatabase.PExecute("UPDATE characters_fake SET level = level+1");
                CharacterDatabase.PExecute("UPDATE characters_fake SET level = level+1, lastup = NOW() WHERE level < 80 AND lastup < (NOW() - INTERVAL {} HOUR) AND HOUR(online) BETWEEN HOUR(NOW()) AND (HOUR(NOW()) + {})", sWorld->getIntConfig(CONFIG_FAKE_WHO_LEVELUP_INTERVAL), sWorld->getIntConfig(CONFIG_FAKE_WHO_ONLINE_INTERVAL));
                }
        }
        else
            m_nextSave -= p_time;
